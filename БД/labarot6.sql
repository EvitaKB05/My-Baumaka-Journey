...
SELECT 
    ...
    RANK() OVER (ORDER BY COUNT(s.id_student) DESC) AS rank_by_students, ---пропускает с одинаковыми значениями--
    DENSE_RANK() OVER (ORDER BY COUNT(s.id_student) DESC) AS dense_rank_by_students, ---не пропускает а присваивает фактическое место в списке--
    ROW_NUMBER() OVER (ORDER BY COUNT(s.id_student) DESC) AS row_num --- просто присваивает уникальный номер по списку---
FROM faculties f
...
SELECT 
    ...
    m.date_m,
    m.mark,
    SUM(m.mark) OVER (PARTITION BY m.id_student, m.id_subject ORDER BY m.date_m, m.id_mark) AS mark_value_sum,
    MAX(m.mark) OVER (PARTITION BY m.id_student, m.id_subject ORDER BY m.date_m, m.id_mark) AS max_mark_so_far,
    ROUND(AVG(m.mark) OVER (PARTITION BY m.id_student, m.id_subject ORDER BY m.date_m, m.id_mark), 2) AS running_avg
FROM marks m
JOIN students s ON m.id_student = s.id_student
JOIN subjects sub ON m.id_subject = sub.id_subject
ORDER BY m.id_student, m.id_subject, m.date_m, m.id_mark;
...
-- 4. Анализ прогресса студентов: сравнение текущей оценки с предыдущей и следующей пока оценок мало, можно подобавлять по приколу--- лаг лид---
SELECT
    s.fio AS student_name,
    ...
    m.date_m,
    m.mark_type,
    FIRST_VALUE(m.mark) OVER (PARTITION BY m.id_student ORDER BY m.date_m) AS first_mark,
    LAG(m.mark) OVER (PARTITION BY m.id_student ORDER BY m.date_m) AS prev_mark,
    LEAD(m.mark) OVER (PARTITION BY m.id_student ORDER BY m.date_m) AS next_mark,
    --m.date_m - LAG(m.date_m) OVER (PARTITION BY m.id_student ORDER BY m.date_m) AS days_between_marks,
    LAST_VALUE(m.mark) OVER (
        PARTITION BY m.id_student 
        ORDER BY m.date_m
...
FROM 
    marks m
JOIN 
    students s ON m.id_student = s.id_student
JOIN 
