...
-- 5. Триггер UPDATE для проверки изменений данных студента fio--
...
-- 6. Триггер для каскадного удаления записей при удалении студента---
...
-- делетнем то, исправили---
DROP EVENT TRIGGER IF EXISTS tr_prevent_ddl_changes;
DROP FUNCTION IF EXISTS prevent_ddl_changes();
...
LANGUAGE plpgsql AS $$
BEGIN
    -- Проверяем тип команды через tg_tag
    IF tg_tag IN (
        'DROP TABLE', 'ALTER TABLE', 'TRUNCATE TABLE',
        'CREATE TABLE', 'CREATE INDEX', 'DROP INDEX',
        'ALTER TYPE', 'DROP TYPE', 'CREATE SCHEMA',
        'DROP SCHEMA', 'ALTER SCHEMA'
    ) THEN
        RAISE EXCEPTION 'Изменение структуры базы данных запрещено! Команда: %', tg_tag;
    END IF;
END;
$$;

-- Создаем триггер события на START команды
CREATE EVENT TRIGGER tr_prevent_ddl_changes
ON ddl_command_start
EXECUTE FUNCTION prevent_ddl_changes();
---чекнем---
ALTER TABLE students ADD COLUMN test_column INT; -- работает, не дает испарвить